      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div className="mt-12 pt-8 border-t border-stone-100 flex flex-col sm:flex-row gap-4">
              {settings.googleFormUrl && (
                <a
                  ...
                  className="flex-1 flex items-center justify-center space-x-2 bg-stone-50 hover:bg-stone-100 text-stone-700 text-sm font-medium py-3.5 px-4 rounded-lg border border-stone-200 transition-colors"
                >
                  <FileText size={16} className="text-stone-500" />
                  <span>구글 폼으로 예약</span>
                </a>
              )}
              {settings.formDownloadUrl && settings.formDownloadUrl !== '#' && (
                <a
          ...
          <motion.div
            ...
            className="bg-white p-10 md:p-12 rounded-2xl shadow-sm border border-ivory-300"
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {submitStatus === 'error' && (
                <div className="p-4 bg-red-50 border border-red-100 text-red-800 rounded-lg text-sm text-center">
                  ⚠️ 문의 발송 중 문제가 발생했습니다. 번거로우시겠지만 좌측의 이메일({settings.contactEmail})로 직접 발송해 주세요.
                </div>
              )}
